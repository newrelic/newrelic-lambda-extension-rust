...
/// Extract function name from Lambda ARN (position 6 in colon-delimited format).
/// Falls back to `config_function_name` if the ARN is empty or malformed.
pub(crate) fn extract_function_name_from_arn<'a>(arn: &'a str, config_function_name: &'a str) -> &'a str {
    if arn.is_empty() {
        warn!("invoked_function_arn is empty, using config fallback: {}", config_function_name);
        return config_function_name;
    }

    // Standard ARN: arn:aws:lambda:{region}:{account}:function:{name}[:version]
    // Position:       0   1     2      3       4         5       6       7
    arn.split(':').nth(6).filter(|s| !s.is_empty()).unwrap_or_else(|| {
        warn!("Could not extract function name from ARN '{}', using config fallback", arn);
...
pub(crate) fn create_newrelic_log_format(
    agent_data: &[u8],
    function_name: &str,
    invoked_function_arn: &str,
    log_group_name: &str,
