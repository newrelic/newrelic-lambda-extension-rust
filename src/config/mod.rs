...
pub fn init_config() -> &'static ExtensionConfig {
    unsafe {
        ...
        let config = GLOBAL_CONFIG.as_ref().unwrap();
        ...
        info!("[Config] Function logs: {}", config.new_relic.send_function_logs);
        info!("[Config] Extension logs: {}", config.new_relic.send_extension_logs);
        info!("[Config] AWS Runtime API: {}", config.aws.runtime_api);
        info!("[Config] Function: {}", config.aws.function_name);
