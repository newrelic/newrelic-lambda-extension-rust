...
impl AwsConfig {
    ...
    fn extract_account_id_from_arn(arn: &str) -> Option<String> {
        ...
        // Zero-alloc: use iterator instead of collecting into Vec
        let mut parts = arn.splitn(6, ':');
        let prefix = parts.next()?;   // "arn"
        let _ = parts.next()?;        // "aws"
        let service = parts.next()?;  // "lambda"
        let _ = parts.next()?;        // region
        let account_id = parts.next()?;
        if prefix == "arn" && service == "lambda" {
...
/// Global configuration instance (safe, no `unsafe` needed)
