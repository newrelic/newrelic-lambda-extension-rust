...
| `NR_TAGS` |  | | Specify tags to be added to all log events. **Optional**. Format: `env:prod;team:myTeam` (colon-delimited key/value, semicolon-delimited pairs).<br><br>**Notes:**<br>- Only affects log events and APM connect Entity Tags.<br>- Does **not** feed the Transaction/Span custom attributes row below — only `NEW_RELIC_LABELS` does. |
...
| `NEW_RELIC_LABELS` |  | | Specify labels in the standard New Relic cross-agent format. **Optional**. Format: `type1:value1;type2:value2` (fixed `:`/`;` delimiters, not affected by `NR_ENV_DELIMITER`).<br><br>**Where it's sent:**<br>- Connect payload's `labels` array → Entity Tags, **unprefixed** (matches the Python/Node.js agents' own behavior).<br>- Forwarded log events → `tags.<type>` attributes.<br>- In APM mode, every Transaction/Span event → `tags.<type>` custom attributes (see next row).<br><br>**Validation:**<br>- Duplicate type: last value wins.<br>- Type/value over 255 chars: truncated, with a warning.<br>- Over 64 entries: list capped, with a warning.<br>- Malformed pair (bad delimiter count, empty type/value): the **entire** list is discarded, with a warning — never a partial list.<br><br>**Notes:**<br>- Independent of `NR_TAGS` — if both are set, both are sent.<br>- `NR_TAGS` is not included in the Transaction/Span attributes — `NEW_RELIC_LABELS` is the only source for those. |
| _(APM mode only)_ Transaction/Span custom attributes | | | `NEW_RELIC_LABELS` is automatically added as `tags.<key>` custom attributes on every Transaction and Span event — equivalent to calling `newrelic.addCustomAttributes()` in every function, with no per-function code.<br><br>**Notes:**<br>- `NR_TAGS` is **not** included, only `NEW_RELIC_LABELS`.<br>- Never overwrites an attribute the agent (or your own `addCustomAttributes()` call) already set on that event.<br>- Only applies when `NEW_RELIC_APM_LAMBDA_MODE=true`; no effect in serverless mode. |
