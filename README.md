...
| `NEW_RELIC_APM_SEND_PLATFORM_METRICS` | `true` | `true`, `false`, `1`, `0` | APM mode only. When `true` (default), Lambda `REPORT` lines are converted to `apm.lambda.*` metrics (duration, billed duration, memory) and sent to the Metric API. Set `false` to stop generating and sending these platform metrics — useful if you only want transaction/error telemetry and not the per-invocation platform metrics. Does not affect error synthesis. |
