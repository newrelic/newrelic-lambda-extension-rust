...
| `NEW_RELIC_RUNTIME_DONE_WAIT_CAP_MS` | `3000` | Number (100–60000) | Hard upper bound in milliseconds on how long the end-of-invocation flush waits for `platform.runtimeDone` before flushing anyway and returning to `GET /next`. On real Lambda `platform.runtimeDone` arrives within a few milliseconds of the handler returning, so this cap is not reached in normal operation. It exists so a silent or absent Lambda Telemetry API — e.g. the local Runtime Interface Emulator (RIE), which never emits telemetry, or a platform stall — cannot park the invocation for the full function deadline, which would otherwise gate the next `GET /next` and hang the invoke response until the sandbox is torn down. The effective wait is `min(remaining deadline, this value)`. Clamped to `[100, 60000]`. |
