...
|--------|-----------|-------------|-------------|
| `NEW_RELIC_EXTENSION_SEND_LOGS` | | `platform`, `extension`, `function`, `all`, or specific `platform.<event>` names | **Unified log configuration** - Send specific log types to New Relic using comma-separated values. Use `all` to send all log types. Examples: `function`, `function,platform`, `all`. To send only specific platform event types instead of all of them, list them by name instead of the bare `platform` keyword, e.g. `platform.report` or `platform.start,platform.report`. The bare `platform` keyword (or `all`) always sends every platform event type and overrides any specific names listed alongside it. This takes precedence over individual log flags below. |
...
| `NEW_RELIC_AWS_ACCOUNT_ID` | | 12-digit AWS account ID | The **AWS** account ID (not your New Relic account ID) used to construct the function ARN for APM entity correlation. Normally the extension derives this from the Extensions API registration response, and failing that from the invoked function ARN — so on real Lambda you never need to set it. Provide it only for environments where neither source supplies an AWS account ID (e.g. the local Runtime Interface Emulator), which otherwise logs `AWS account ID not available from registration, using placeholder`. A value obtained from registration or the ARN takes precedence over this variable. |
