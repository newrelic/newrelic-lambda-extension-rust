...
| `NR_TAGS` |  | | Specify tags to be added to all log events. **Optional**. Each tag is composed of a colon-delimited key and value. Multiple key-value pairs are semicolon-delimited; for example, `env:prod;team:myTeam`. Scope is intentionally limited to this and the APM connect payload's Entity Tags — `NR_TAGS` does **not** feed the Transaction/Span custom attributes below; only `NEW_RELIC_LABELS` does. |
...
| `NEW_RELIC_LABELS` |  | | Specify labels in the standard New Relic cross-agent format. **Optional**. Format is `type1:value1;type2:value2` (fixed `:`/`;` delimiters — not affected by `NR_ENV_DELIMITER`). Labels are sent unprefixed in the APM connect payload's `labels` array (Entity Tags), and as `tags.<type>` attributes on every forwarded log event — matching the official New Relic Python agent's behavior for each. Independent of `NR_TAGS` — if both are set, both are sent. A duplicate type keeps only its last value; each type/value is truncated at 255 characters with a warning; the list is capped at 64 entries with a warning; a malformed pair (wrong delimiter count, empty type, or empty value) discards the *entire* list with a warning rather than sending a partial one. In APM mode, also added as `tags.<type>` custom attributes on every Transaction and Span event (see below) — `NEW_RELIC_LABELS` is the *only* source for this, `NR_TAGS` is not included. |
| _(APM mode only)_ Transaction/Span custom attributes | | | `NEW_RELIC_LABELS` (only — not `NR_TAGS`) is automatically added as `tags.<key>` custom attributes on every Transaction (`analytic_event_data`) and Span (`span_event_data`) event — the same effect as calling `newrelic.addCustomAttributes({"tags.<key>": value})` in every function, without any per-function code. The `tags.` prefix matches the log-forwarding attribute convention above, for uniformity across everything the extension adds; Entity Tags stay unprefixed since that's a structured `label_type`/`label_value` field, not a flat attribute map. An attribute the agent (or the customer's own `addCustomAttributes()` call) already set on a given event is never overwritten. Only applies when `NEW_RELIC_APM_LAMBDA_MODE=true`; has no effect in serverless/log-forwarding mode. |
