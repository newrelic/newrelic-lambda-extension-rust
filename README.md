...
| `NEW_RELIC_APM_BLOCKING_AGENT_PAYLOAD` | `false` | `true`, `false`, `1`, `0` | When `true`, the extension holds `/next` until the agent's telemetry payload for the current invocation arrives (or `NEW_RELIC_APM_AGENT_PAYLOAD_TIMEOUT_MS` / the remaining invoke deadline is exhausted). Delivers telemetry within the same invoke rather than waiting for the next invocation or `SHUTDOWN` — useful for sparse-traffic functions where infrequent invocations mean data would otherwise sit for several minutes. When `false` (default), behavior is unchanged: a late-arriving payload is picked up on the next invocation or at `SHUTDOWN`. Takes precedence over `NEW_RELIC_EXTENSION_PIPELINE_FLUSH` on invocations where the wait is exercised — you will not get pipeline flush's billed-duration savings on those invocations, since the delivery guarantee wins. |
| `NEW_RELIC_APM_AGENT_PAYLOAD_TIMEOUT_MS` | `200` | Number (0–2000) | Maximum milliseconds to wait for the agent's payload when `NEW_RELIC_APM_BLOCKING_AGENT_PAYLOAD` is `true`. Ignored otherwise. Clamped to `[0, 2000]`. |
